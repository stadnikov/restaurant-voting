...
class UserFoodControllerTest extends AbstractControllerTest {
    ...
    void getRestaurantTodayFood() throws Exception {
        ResultActions action = perform(MockMvcRequestBuilders.get(REST_URL + SLASH_RID + SLASH_MENU))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(RestaurantTestData.RESTAURANT_MATCHER_WITH_FOOD.contentJson(RestaurantTestData.RESTAURANT_2));
        // TODO investigate following code
//        Restaurant restaurant = FoodTestData.RESTAURANT_WITH_FOOD_MATCHER.readFromJson(action);
//        RestaurantTestData.RESTAURANT_MATCHER.assertMatch(RestaurantTestData.RESTAURANT_2, restaurant);
//        FoodTestData.FOOD_MATCHER.assertMatch(FoodTestData.RESTAURANT_2_FOOD, restaurant.getFood());
    }
